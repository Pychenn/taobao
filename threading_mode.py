...
Note：用生产者模型代替request请求模块，处理速度0.3s-0.6s
      用消费者模型代替正则解析模块，处理速度0.006s-0.007s
      如果模拟总共请求1000个页面，用100个线程进行请求处理，2个线程进行解析处理，所需时间约6.5s
      如果模拟总共请求1000个页面，用100个线程进行请求处理，1个线程进行解析处理，所需时间约7.5s
'''
...
def main():
    ...
    global html_queue
    html_queue = queue.Queue(100)   #创建存放html的队列，最大值100
    name = "男装"
    threads = []    #存放所有线程的队列

    print("work start. Time：",time.clock())
    for p in range(1000):   #总共需要请求的页面数，创建url队列
        url_queue.put(p,1)
